...
export default defineUserConfig({
  ...
  locales: {
    '/': {
      title: 'Web 3.0 and Post-Quantum Security ',
      lang: 'en-US',
  ...
  bundler: viteBundler({
    vueCompilerOptions: {
      isCustomElement: (tag) => tag === 'Custom',
    },
  }),
  shouldPrefetch: false, // 站点较大，页面数量较多时，不建议启用
  plugins: [
    registerComponentsPlugin({
      componentsDir: path.resolve(__dirname, 'components'), // 关键路径配置
      componentsPatterns: ['**/*.vue'], // 默认值，可省略
      getComponentName: (filename) => 
        filename.replace(/\.vue$/, '') // 自动转换文件名到组件名
    }),
  ],
  theme: plumeTheme({
